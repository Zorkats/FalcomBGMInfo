...
void Log(const std::string& message) {
    if (g_modDirectory.empty()) g_modDirectory = ResolveModDirectory();
    std::lock_guard<std::mutex> lock(g_LogMutex);
    std::string path = g_modDirectory + "\\mod_log.txt";
...
BOOL WINAPI Detour_GetKeyboardState(PBYTE lpKeyState) {
    if (g_showMenu && lpKeyState) { memset(lpKeyState, 0, 256); return TRUE; }
...
void DetectAndConfigure() {
    ...
    if (exe.find("xanadu.exe") != std::string::npos) {
        ...
        g_Config.signature = "55 8B EC 83 EC 08 53 56 8B F1 57 8B 7E 18";
    }
        ...
        g_Config.signature = "55 8B EC 51 53 56 57 8B 7D 08 8B 75 0C";
    }
...
void InitImGui(IDirect3DDevice9* pDevice) {
    ...
    if (g_pMenuFont) {
        ImFontConfig cfg1; cfg1.MergeMode = true;
        io.Fonts->AddFontFromFileTTF(f2.c_str(), 18.0f, &cfg1, r);
    }
    
    g_pToastFont = io.Fonts->AddFontFromFileTTF(f1.c_str(), 28.0f);
    if (g_pToastFont) {
        ImFontConfig cfg2; cfg2.MergeMode = true;
        io.Fonts->AddFontFromFileTTF(f2.c_str(), 28.0f, &cfg2, r);
    }
...
HRESULT WINAPI My_EndScene(IDirect3DDevice9* pDevice) {
    ...
    if (dW != g_lastDisplayWidth || dH != g_lastDisplayHeight) {
        g_lastDisplayWidth = dW;
        g_lastDisplayHeight = dH;
    ...
    if ((g_toastTimer > 0.0f || g_toastCurrentX != -10000.0f) && !g_currentBgmInfo.songName.empty()) {
        ...
        int LC = 0;
        if (!g_currentBgmInfo.songName.empty()) LC++;
        if (g_ModConfig.showJapanese && !g_currentBgmInfo.japaneseName.empty()) LC++;
        if (!dt.empty()) LC++;
        if (!g_currentBgmInfo.album.empty()) LC++;
        
        float TH = LH * (float)LC + (LH * 0.2f * (LC - 1)), TTH = TH + (TY * 2.0f), NIW = TTH, BW = TW + (TX * 2.0f), TTW = NIW + BW;
        float target = io.DisplaySize.x - TTW - SP;
        if (target < SP) target = SP;
        ...
        if (g_toastCurrentX != -10000.0f) {
            ...
            if (g_pToastTexture) {
                dl->AddImage((void*)g_pToastTexture, ImVec2(g_toastCurrentX, SP), ImVec2(g_toastCurrentX + NIW, SP + TTH));
            }
            
            float cY = SP + (TTH - TH) * 0.5f;
            auto DrawLine = [&](const std::string& s, ImU32 col) {
                if (!s.empty()) {
                    dl->AddText(ImVec2(p_box.x + TX, cY), col, s.c_str());
                    cY += LH * 1.2f;
                }
            };
    ...
    pDevice->CreateStateBlock(D3DSBT_ALL, &pSB);
    IDirect3DSurface9* pRT = nullptr;
    
    if (SUCCEEDED(pDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pRT))) {
    ...
    if (f && IsTargetW(f)) {
        if (g_bufferMutex.try_lock()) {
            if (!g_bNewBgmAvailable) {
                WCharToString(f, g_bgmFilenameBuffer, MAX_PATH);
...
HANDLE WINAPI Detour_CreateFileA(LPCSTR f, DWORD a, DWORD s, LPSECURITY_ATTRIBUTES sec, DWORD d, DWORD fl, HANDLE t) {
    if (f && IsTarget(f)) {
        if (g_bufferMutex.try_lock()) {
            if (!g_bNewBgmAvailable) {
                strcpy_s(g_bgmFilenameBuffer, MAX_PATH, f);
                g_bNewBgmAvailable = true;
            }
            g_bufferMutex.unlock();
        }
...
void ProcessBgmTrigger(const std::string& fn) {
    ...
    for (auto& e : g_bgmMap) {
        ...
        if (input.length() >= k.length() && input.compare(input.length() - k.length(), k.length(), k) == 0) {
            ...
            bool show = g_ModConfig.ignoreCooldown || 
                        (g_songLastShown.find(e.second.songName) == g_songLastShown.end()) || 
...
void __stdcall ProcessXanaduLogic(int id) {
...
bool __stdcall TryExtract(DWORD addr) {
    if (addr < 0x10000 || addr > 0x7FFFFFFF || IsBadReadPtr((void*)addr, 4)) return false;
    char* p = (char*)addr;
    if (p[0] < 32 || p[0] > 126) return false;
    
    size_t l = 0;
    while (l < MAX_PATH && !IsBadReadPtr(&p[l], 1) && p[l] != 0) l++;
...
IDirect3D9* WINAPI Detour_D3DCreate9(UINT v) {
    ...
    if (g_Config.useRvaHook) {
        uintptr_t target = !g_Config.signature.empty() ? MemoryScanner::Scan(g_Config.signature) : 0;
        if (!target && g_Config.bgmFuncRVA != 0) {
            target = (uintptr_t)GetModuleHandle(NULL) + g_Config.bgmFuncRVA;
        }
        ...
        if (target) {
            ...
            MH_CreateHook((LPVOID)target, detourFunc, originalFunc);
...
BOOL WINAPI DllMain(HMODULE h, DWORD r, LPVOID res) {
    if (r == DLL_PROCESS_ATTACH) {
        ...
        while (!d3d) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
            d3d = GetModuleHandleA("d3d9.dll");
        }
        
        void* pC = (void*)GetProcAddress(d3d, "Direct3DCreate9");
        if (pC) {
            MH_CreateHook(pC, &Detour_D3DCreate9, (LPVOID*)&g_pfnOriginalDirect3DCreate9);
            MH_EnableHook(pC);
        }
        
        HMODULE u32 = GetModuleHandleA("user32.dll");
        MH_CreateHook(GetProcAddress(u32, "SetCursorPos"), &Detour_SetCursorPos, (LPVOID*)&g_pfnOriginalSetCursorPos);
        MH_CreateHook(GetProcAddress(u32, "ClipCursor"), &Detour_ClipCursor, (LPVOID*)&g_pfnOriginalClipCursor);
        MH_CreateHook(GetProcAddress(u32, "GetAsyncKeyState"), &Detour_GetAsyncKeyState, (LPVOID*)&g_pfnOriginalGetAsyncKeyState);
        MH_CreateHook(GetProcAddress(u32, "GetKeyState"), &Detour_GetKeyState, (LPVOID*)&g_pfnOriginalGetKeyState);
        MH_CreateHook(GetProcAddress(u32, "GetKeyboardState"), &Detour_GetKeyboardState, (LPVOID*)&g_pfnOriginalGetKeyboardState);
        MH_EnableHook(MH_ALL_HOOKS);
